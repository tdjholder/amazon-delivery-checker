...
def poll():
    ...
    time.sleep(2)
    sub_button = driver.find_element_by_id("subsContinueButton")
    sub_button.click()
    ...
    while True:
        ...
        body_text = body.text
        if "No delivery windows available." in body_text:
            count += 1
            print("No deliveries found. Attempt: %d" % count)
            time.sleep(30)
            driver.refresh()
        else:
            if "sorry we are unable to fulfill your entire order" in body_text:
                continue_bottom = driver.find_element_by_name("continue-bottom")
